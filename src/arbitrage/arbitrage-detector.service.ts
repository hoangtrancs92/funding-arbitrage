...
export class ArbitrageDetector {
  ...
  detectOpportunities(
    ...
    for (const symbol of allSymbols) {
      const symbolRates = this.getSymbolRatesAcrossExchanges(
        symbol,
        fundingRatesMap,
      );

      if (symbolRates.length >= 2) {
        const symbolOpportunities = this.findArbitrageForSymbol(
          symbol,
          symbolRates,
        );
  ...
  analyzeOpportunity(
    ...
    if (
      opportunity.expectedReturn > this.MIN_EXPECTED_RETURN &&
      overallRisk < this.MAX_RISK_SCORE
    ) {
      if (
        marketConditions.liquidity > 0.7 &&
        marketConditions.spreadStability > 0.6
      ) {
        ...
        reasoning.push(
          'Good opportunity but market conditions need improvement',
        );
      }
    } else {
