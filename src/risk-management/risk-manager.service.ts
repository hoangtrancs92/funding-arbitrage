...
import { RiskParameters, RiskMetrics, PositionRisk, RiskAlert } from './risk.interface';
...
export class RiskManager {
  ...
  evaluateNewPosition(
    ...
    if (positionSize > this.riskParameters.maxPositionSize) {
      canOpen = false;
      reasons.push(`Position size ${positionSize} exceeds maximum ${this.riskParameters.maxPositionSize}`);
    }
    
    // Check max open positions
    if (currentPositions.length >= this.riskParameters.maxOpenPositions) {
      canOpen = false;
      reasons.push(`Maximum number of positions reached (${this.riskParameters.maxOpenPositions})`);
    }
    ...
    if (portfolioRisk > this.riskParameters.maxPortfolioRisk) {
      canOpen = false;
      reasons.push(`Position would exceed max portfolio risk: ${(portfolioRisk * 100).toFixed(2)}% > ${(this.riskParameters.maxPortfolioRisk * 100).toFixed(2)}%`);
    }
    
    // Check correlation with existing positions
    const correlation = this.calculateSymbolCorrelation(symbol, currentPositions);
    if (correlation > this.riskParameters.correlationLimit) {
      canOpen = false;
      reasons.push(`High correlation ${(correlation * 100).toFixed(2)}% with existing positions`);
    }
  ...
  calculateRiskMetrics(
    ...
    const totalExposure = positions.reduce((sum, pos) => sum + Math.abs(pos.size * pos.markPrice), 0);
    const currentLeverage = portfolioValue > 0 ? totalExposure / portfolioValue : 0;
    ...
    const leverageScore = Math.min(currentLeverage / this.riskParameters.maxLeverage, 1);
    const exposureScore = Math.min(totalExposure / (portfolioValue * 5), 1); // Assume 5x as max reasonable exposure
    const drawdownScore = Math.abs(Math.min(dailyPnl, 0)) / this.riskParameters.maxDailyLoss;
    
    const riskScore = (leverageScore * 0.4 + exposureScore * 0.3 + drawdownScore * 0.3);
  ...
  calculatePositionRisks(positions: Position[]): PositionRisk[] {
    return positions.map(position => {
      ...
      const distanceToLiquidation = position.side === 'LONG' 
        ? (position.markPrice - (position.entryPrice * 0.9)) / position.markPrice
        : ((position.entryPrice * 1.1) - position.markPrice) / position.markPrice;
  ...
  monitorRisks(
    ...
    if (riskMetrics.currentLeverage > this.riskParameters.maxLeverage * 0.8) {
      newAlerts.push({
        ...
        type: 'LEVERAGE',
        severity: riskMetrics.currentLeverage > this.riskParameters.maxLeverage ? 'CRITICAL' : 'HIGH',
    ...
    if (riskMetrics.dailyPnl < -this.riskParameters.maxDailyLoss * 0.8) {
      newAlerts.push({
        ...
        type: 'DRAWDOWN',
        severity: riskMetrics.dailyPnl < -this.riskParameters.maxDailyLoss ? 'CRITICAL' : 'HIGH',
        message: `Daily loss ${riskMetrics.dailyPnl.toFixed(2)} approaching limit ${-this.riskParameters.maxDailyLoss}`,
        recommendedActions: ['Stop trading for today', 'Review strategy parameters'],
        timestamp: new Date(),
        acknowledged: false,
      });
    }
    ...
    for (const positionRisk of positionRisks) {
      if (positionRisk.distanceToLiquidation && positionRisk.distanceToLiquidation < 0.1) {
        newAlerts.push({
          id: `LIQUIDATION_${positionRisk.positionId}_${Date.now()}`,
          type: 'POSITION_SIZE',
          severity: positionRisk.distanceToLiquidation < 0.05 ? 'CRITICAL' : 'HIGH',
          ...
          recommendedActions: ['Add margin', 'Reduce position size', 'Close position'],
          timestamp: new Date(),
          acknowledged: false,
        });
      }
  ...
  private calculateVaR(positions: Position[], confidence: number): number {
    ...
    const totalValue = positions.reduce((sum, pos) => sum + Math.abs(pos.size * pos.markPrice), 0);
  ...
  private calculateSymbolCorrelation(symbol: string, positions: Position[]): number {
