...
export class FundingRateService {
  ...
  async collectFundingRates(
    ...
    try {
      ...
      this.logger.log(
        `Collected ${binanceRates.length} funding rates from Binance`,
      );
    } catch (error) {
    ...
    try {
      ...
      this.logger.log(
        `Collected ${bybitRates.length} funding rates from Bybit`,
      );
    } catch (error) {
  ...
  async findArbitrageOpportunities(symbols?: string[]) {
    ...
    const opportunities =
      this.arbitrageDetector.detectOpportunities(fundingRatesMap);
    this.logger.log(
  ...
  async getTopOpportunities(limit = 10) {
    ...
    return {
      opportunities: result.opportunities.slice(0, limit).map((opp) => ({
        ...opp,
  ...
  async startMonitoring(intervalMinutes = 5) {
    ...
    const monitor = async () => {
      try {
        ...
        // Log summary
        const executableOpportunities = result.analyses.filter(
          (a) => a.recommendation === 'EXECUTE',
        );
        this.logger.log(
          `Monitoring update: ${result.opportunities.length} opportunities found, ${executableOpportunities.length} executable`,
        );
