...
import {
  FundingArbitrageScenario,
  AutoTradeConfig,
  TradeExecution,
} from './auto-trade.interface';
...
export class AutoTradeScheduler {
    ...
    scenarios: [
      {
        ...
        timing:
          'Vào trước snapshot 2-3 phút, thoát sau khi cả 2 sàn trả funding',
      ...
      },
      {
        ...
        description:
          'Cả hai sàn cùng dương hoặc cùng âm, nhưng chênh lệch ≥ 0.25%',
        ...
        timing:
          'Vào trước funding gần nhất, thoát sau khi sàn funding cao trả tiền',
      ...
      },
      {
        ...
        description:
          'Cả hai sàn có funding cùng dấu, nhưng giá futures chênh nhau ≥ 0.25%',
        condition: 'Long sàn giá thấp + Short sàn giá cao',
        strategy: 'Long sàn giá thấp + Short sàn giá cao',
        timing:
          'Ngay khi phát hiện gap ≥ 0.25%, thoát khi gap thu hẹp hoặc sau funding',
        minProfitThreshold: 0.0025, // 0.25%
        riskLevel: 'MEDIUM',
      },
      {
        ...
        condition:
          'Mở Long/Short hedge cùng lúc - đóng sàn trả funding trước, rồi sàn còn lại sau',
        strategy:
          'Mở Long/Short hedge cùng lúc - đóng sàn trả funding trước, rồi sàn còn lại sau',
      ...
      },
      {
        ...
        description:
          'Cả hai sàn funding đều cao (≥ 0.4%) cùng chiều, gap giá nhỏ',
        condition:
          'Long cả hai sàn (nếu funding âm) hoặc Short cả hai sàn (nếu funding dương)',
        strategy:
          'Long cả hai sàn (nếu funding âm) hoặc Short cả hai sàn (nếu funding dương)',
  ...
  async scanForOpportunities() {
    ...
    try {
      ...
      this.tradingGateway?.broadcastFundingRatesUpdate(
        Array.from(fundingRates.keys()),
      );
      ...
      const bestOpportunities = OpportunityFilter.getTopOpportunities(
        this.rawOpportunities,
        5,
      );

      // Broadcast optimized opportunities
      this.tradingGateway?.broadcastOpportunitiesUpdate(
        Array.from(fundingRates.keys()),
      );
  ...
  private async collectScenarioOpportunities(
    ...
    switch (scenario.id) {
      case 1:
        ...
        break;
      case 2:
        await this.collectSameSignDifferentRateOpportunities(
          scenario,
          fundingRates,
        );
        break;
      ...
      case 4:
        ...
        break;
      case 5:
        await this.collectHighSameDirectionOpportunities(
          scenario,
          fundingRates,
        );
        break;
  ...
  private async collectOppositeSignOpportunities(
    scenario: FundingArbitrageScenario,
    fundingRates: Map<string, any[]>,
  ) {
        ...
        for (const symbol of commonSymbols) {
          ...
          if (this.isOppositeSign(rate1.fundingRate, rate2.fundingRate)) {
            ...
            if (expectedProfit >= scenario.minProfitThreshold) {
              ...
              const longExchange =
                rate1.fundingRate < 0 ? exchange1 : exchange2;
              const shortExchange =
                rate1.fundingRate < 0 ? exchange2 : exchange1;
              const longFundingRate =
                rate1.fundingRate < 0 ? rate1.fundingRate : rate2.fundingRate;
              const shortFundingRate =
                rate1.fundingRate < 0 ? rate2.fundingRate : rate1.fundingRate;
  ...
  private async collectSameSignDifferentRateOpportunities(
    scenario: FundingArbitrageScenario,
    fundingRates: Map<string, any[]>,
  ) {
        ...
        for (const symbol of commonSymbols) {
          ...
          if (this.isSameSign(rate1.fundingRate, rate2.fundingRate)) {
            ...
            if (expectedProfit >= scenario.minProfitThreshold) {
              ...
              const longExchange =
                rate1.fundingRate < rate2.fundingRate ? exchange1 : exchange2;
              const shortExchange =
                rate1.fundingRate < rate2.fundingRate ? exchange2 : exchange1;
  ...
  private async collectPriceGapOpportunities(
    scenario: FundingArbitrageScenario,
    fundingRates: Map<string, any[]>,
  ) {
  ...
  private async collectTimingDesyncOpportunities(
    scenario: FundingArbitrageScenario,
    fundingRates: Map<string, any[]>,
  ) {
  ...
  private async collectHighSameDirectionOpportunities(
    scenario: FundingArbitrageScenario,
    fundingRates: Map<string, any[]>,
  ) {
  ...
  private async closeExpiredPositions() {
    ...
    const expiredPositions = this.activePositions.filter(
      (p) => p.status === 'ACTIVE' && this.shouldClosePosition(p),
    );
  ...
  private async executeTopOpportunities(
    bestOpportunities: SimpleOpportunity[],
  ) {
  ...
  private async executeOptimizedTrade(opportunity: SimpleOpportunity) {
    try {
      ...
      this.logger.log(
        `✅ Trade executed successfully for ${opportunity.symbol}`,
      );
    } catch (error) {
      this.logger.error(
  ...
  private async closePosition(position: TradeExecution) {
    try {
      this.logger.log(
        `🔄 Closing position: ${position.symbol} (${position.scenarioId})`,
      );
  ...
  getStatus() {
    return {
      ...
      activePositions: this.activePositions.filter((p) => p.status === 'ACTIVE')
        .length,
      dailyPnL: this.dailyPnL,
      scenarios: this.config.scenarios.map((s) => ({
        id: s.id,
        name: s.name,
        activePositions: this.activePositions.filter(
          (p) => p.scenarioId === s.id && p.status === 'ACTIVE',
        ).length,
      })),
    };
  }
  ...
  updateScenarioConfig(
    scenarioId: number,
    updates: Partial<FundingArbitrageScenario>,
  ) {
  ...
  startAutoTrading(intervalMinutes = 1) {
    ...
    this.logger.log(
      `🚀 Starting auto trading with ${intervalMinutes} minute intervals`,
    );
    this.config.enabled = true;
